...
public class FGListPopup : FGPopupWindow
{
	...
	private static void LoadSymbolIcons()
	{
		...
		var oneForAll = new HashSet<SymbolKind> { SymbolKind.Namespace, SymbolKind.EnumMember, SymbolKind.Parameter,
			...
			SymbolKind.TypeParameter, SymbolKind.Label, SymbolKind.LocalConstant };
	...
	public static FGListPopup Create(FGTextEditor editor, Rect buttonRect)
	{
		...
		var onToken = editor.TextBuffer.GetTokenAt(editor.caretPosition, out lineIndex, out tokenIndex, out atTokenBeginning);
		if (onToken != null && /*!atTokenBeginning &&*/ onToken.tokenKind >= SyntaxToken.Kind.Keyword)
		{
			// eat
		...
		{
			if (!atTokenBeginning && onToken.tokenKind == SyntaxToken.Kind.Comment)
			{
			//	Debug.LogWarning("Cancelling autocomplete!!!");
				return null;
			}
			if (onToken != null && (
		...
		FGListPopup window = CreateInstance<FGListPopup>();
		...
		Vector2 screenPoint = GUIUtility.GUIToScreenPoint(new Vector2(buttonRect.x, buttonRect.yMax));
		Rect position = new Rect(screenPoint.x - 24f - FGTextEditor.charSize.x * typedInPart.Length, screenPoint.y, 1f, 21f);
		window.dropDownRect = new Rect(position.x, position.y - FGTextEditor.charSize.y, 1f, FGTextEditor.charSize.y);
		window.position = position;
		window.TypedInPart = typedInPart;

	//	Debug.Log("tokenLeft: " + tokenLeft);
	//	Debug.Log("onToken: " + onToken);
	...
	private void OnGUI()
	{
		...
		GUI.Box(scrollViewRect, GUIContent.none, textEditor.styles.scrollViewStyle);
		...
		if (Event.current.type == EventType.Repaint)
		{
			...
			for (var i = Mathf.Min(filteredData.Count, scrollPosition + maxListItems) - 1; --i >= scrollPosition; )
			{
				...
				var asType = filteredData[i] as TypeDefinition;
				var asMethod = filteredData[i] as MethodDefinition;
				var generic =
					asType != null ? asType.typeParameters != null :
					asMethod != null ? asMethod.typeParameters != null : false;
				if (generic)
					itemContent.text += "<>";
			...
			for (var i = Mathf.Min(filteredData.Count, scrollPosition + maxListItems + 1); --i >= scrollPosition; )
			{
				...
				var styledName = filteredData[i].name;
				if (matcher != null)
				{
	...
	//private static MethodInfo setAlphaMethod;
	//private static FieldInfo parentField;
	//private object containerWindow;

	//private void SetAlpha(float alpha)
	//{
	//	if (containerWindow == null)
	//	{
	//		parentField = typeof(EditorWindow).GetField("m_Parent", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
	//		if (parentField != null)
	//		{
	//			object parentView = parentField.GetValue(this);
	//			if (parentView != null)
	//			{
	//				var windowField = typeof(Editor).Assembly.GetType("UnityEditor.View").GetField("m_Window", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
	//				if (windowField != null)
	//				{
	//					containerWindow = windowField.GetValue(parentView);
	//					if (containerWindow != null)
	//					{
	//						setAlphaMethod = containerWindow.GetType().GetMethod("SetAlpha", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
	//					}
	//				}
	//			}
	//		}
	//	}
	//	if (containerWindow != null && setAlphaMethod != null)
	//	{
	//		setAlphaMethod.Invoke(containerWindow, new object[] { alpha });
	//	}
	//}
	...
	public SymbolDefinition OnOwnerGUI()
	{
		...
		//if (Event.current.control != ctrlIsDown)
		//{
		//	ctrlIsDown = !ctrlIsDown;
		//	SetAlpha(ctrlIsDown ? 0.4f : 1f);
		//}
		...
		if (Event.current.type == EventType.keyDown)
		{
				...
				currentItem >= 0 && ("\t\n {}[]().,:;+-*/%&|^!~=<>?@#\'\"\\".IndexOf(Event.current.character) >= 0 || Event.current.keyCode == KeyCode.KeypadEnter))
			{
				...
				var completion = filteredData[currentItem].name;
