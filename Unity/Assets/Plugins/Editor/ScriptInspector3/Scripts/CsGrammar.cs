	...
	//	parser = new Parser(r_compilationUnit, this);
		...
		parser.Add(new Rule("classMemberDeclaration",
			new Seq(
				...
				| new Seq("partial", "void", methodDeclaration | interfaceDeclaration | classDeclaration | structDeclaration)
...
//	| 'struct' ;
	...
	//	var declarationStatement = new Id("declarationStatement");
		...
		parser.Add(new Rule("statement",
			new If(type - IDENTIFIER, localVariableDeclaration - ";")
	...
	//		));
		...
		parser.Add(new Rule("operatorDeclarator",
			new Seq( "operator",
				new Seq( new Lit("+") | "-", "(", type, IDENTIFIER, binaryOperatorPart | unaryOperatorPart )
				| overloadableUnaryOperator - "(" - type - IDENTIFIER - unaryOperatorPart
				| overloadableBinaryOperator - "(" - type - IDENTIFIER - binaryOperatorPart)
		...
		parser.Add(new Rule("conversionOperatorDeclaration",
			new Seq(conversionOperatorDeclarator, operatorBody)
			));

		parser.Add(new Rule("conversionOperatorDeclarator",
			new Seq( new Lit("implicit") | "explicit", "operator", type, "(", type, IDENTIFIER, ")" )
		...
		parser.Add(new Rule("attributes",
			new Many(
				"[" - new Opt(new Id("attributeTargetSpecifier")) - attribute -
					new Many("," - attribute) - "]"
			)
			) { semantics = SemanticFlags.AttributesScope });

		parser.Add(new Rule("attributeTargetSpecifier",
			(new Lit("field") | "event" | "method" | "param" | "property" | "return" | "type")
				- ":"));

		parser.Add(new Rule("attribute",
			typeName - new Opt(arguments)
			| ".ATTRIBUTE"
			));
...
//*
		parser.Add(new Rule("primaryExpression",
			new Seq( primaryExpressionStart, new Many(primaryExpressionPart) )
			| new Seq(
				"new",
//					new If(delegateCreationExpression, delegateCreationExpression)
				nonArrayType - (
		...
		parser.Add(new Rule("qidStart",
			...
			| (IDENTIFIER | "global") - new Opt("::" - IDENTIFIER)
			));
		...
		parser.Add(new Rule("objectInitializer",
			new Opt(memberInitializerList) - new Opt(",") - "}"
			));
		...
		parser.Add(new Rule("memberInitializer",
			IDENTIFIER - "=" - (expression | objectOrCollectionInitializer)
			));
		...
		parser.Add(new Rule("implicitAnonymousFunctionParameterList",
			new Seq( implicitAnonymousFunctionParameter, new Many(new Seq(",", implicitAnonymousFunctionParameter)) )
			));
...
//*/
	...
	//	int nonTriviaTokenIndex = -1;
		...
		public bool CollectCompletions(TokenSet tokenSet)
		{
			...
			if (tokenSet.Matches(Instance.tokenIdentifier))
			{
		...
		public void InsertMissingToken(string errorMessage)
		{
			...
			var missingToken = new SyntaxToken(SyntaxToken.Kind.Missing, string.Empty) { style = null };
