		...
		switch (Event.current.type)
		{
			case EventType.KeyDown:
				...
				else if (Event.current.alt && !Event.current.shift && !EditorGUI.actionKey && Event.current.keyCode == KeyCode.Return)
				{
					...
					{
						var isMaximized = (bool) API.isMaximizedMethod.Invoke(null, new object[] {this});
						if (isMaximized)
							API.unMaximizeMethod.Invoke(null, new object[] {this});
						else
							API.maximizeMethod.Invoke(null, new object[] {this});
					}
			#else
					maximized = !maximized;
			#endif
