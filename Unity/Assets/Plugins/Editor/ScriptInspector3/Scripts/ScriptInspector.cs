 ...
 * 
 * Unity's legendary editor for C#, UnityScript and Boo scripts, Shaders, and text, now transformed into an advanced C# IDE!!!
 * 
 * NOTE TO UNITY 3.5 USERS:
 * This package has been exported from Unity 4.0.0 to make use of the new Font Importer features. Other than that,
 * there's no other reason why Si3 wouldn't run in Unity 3.5! If you want to use Si3 with Unity 3.5 simpy copy the
 * Assets/Plugins/Editor/ScriptInspector3 folder manually from a Unity 4+ project to your Unity 3.5 project.
 * 
 * New in v3.0:
 * - To be added...
 * 
...
public class ScriptInspector : Editor
{
	...
	protected static EditorWindow GetCurrentInspector()
	{
		...
		var wndType = typeof(EditorWindow).Assembly.GetType("UnityEditor.InspectorWindow");
		if (wndType != null)
		{
			if (currentInspectorWindowField == null)
				currentInspectorWindowField = wndType.GetField("s_CurrentInspectorWindow",
