...
public enum SemanticFlags
{
	...
	QueryBodyScope                = 34 << 8,
	// TODO: Add query expression scopes
		...
		public SymbolDefinition resolvedSymbol {
			get {
				if (_resolvedSymbol != null &&
					_resolvedVersion != 0 && (_resolvedVersion != resolverVersion
					|| !_resolvedSymbol.IsValid()
				))
			...
			if (numValidNodes < nodes.Count)
			{
				...
				if (reusable != null)
				{
					...
					//		+ scanner.CurrentTokenIndex() + " vs. " + reusable.RuleName);
						if (firstLeaf == null || firstLeaf.line <= scanner.CurrentLine() - 1)
						{
							reusable.Dispose();
					...
					else
					{
						...
						else if (firstLeaf == null || firstLeaf.syntaxError != null)
						{
						...
						else if (scanner.Current != null && firstLeaf.line <= scanner.CurrentLine() - 1)
						{
						//	Debug.Log("Cannot reuse " + rule.GetName() + " at line " + scanner.CurrentLine() + ":"
						//		+ scanner.CurrentTokenIndex() + " (" + scanner.Current.text + "...) ");
							reusable.Dispose();
							if (firstLeaf.line == scanner.CurrentLine() - 1)
							{
								removedReusable = true;
							}
							else
