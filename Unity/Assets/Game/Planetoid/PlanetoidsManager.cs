...
public class PlanetoidsManager : MonoBehaviour {
	...
	public static PlanetoidsManager Instance;
	...
	void Awake () {
		...
		else throw new UnityException("More than one PlanetoidsManager in the scene!");
	}
	
	void Start () {
		for (int i = 0; i < _initialPlanetoidCount; ++i) {
			CreatePlanetoid();
		}
	}
	...
	void CreatePlanetoid(Vector2 pos) {
	...
	public Planetoid GetNewOrCashedPlanetoid() {
		if (_cashedPlanetoids.Count > 0) {
			Planetoid cashed = _cashedPlanetoids[0];
			cashed.gameObject.SetActive(true);
			_cashedPlanetoids.Remove(cashed);
			_planetoids.Add(cashed);
			return cashed;
		}
		else {
			...
			Planetoid newPlanetoid = newPlanetoidObject.GetComponent<Planetoid>();
			_planetoids.Add(newPlanetoid);
			return newPlanetoid;
		}
	}
